...
class User extends CatchController
{
    ...
    {
        return CatchResponse::paginate($this->user->getListAdmin());
    }
   ...
   */
    ...
    public function save(CreateRequest $request)
    {
        ...
        $this->user->attachRoles($request->post('roles'));

        if ($request->post('jobs')) {
            $this->user->attachJobs($request->post('jobs'));
        }
    ...
    {
        $this->user->updateBy($id, $request->filterEmptyField()->post());
        ...
        if (!empty($request->post('roles'))) {
            $user->attachRoles($request->post('roles'));
        }
        if (!empty($request->post('jobs'))) {
            $user->attachJobs($request->post('jobs'));
        }

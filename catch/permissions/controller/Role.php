...
class Role extends CatchController
{
    ...
    {
        if (Roles::where('identify', $request->post('identify'))->where('id', '<>', $id)->find()) {
            throw new FailedException('角色标识 [' . $request->post('identify') . ']已存在');
        }

        $this->role->updateBy($id, $request->post());
        $role = $this->role->findBy($id);

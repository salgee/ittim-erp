...
class CatchAuth
{
    ...
    public function attempt($condition)
    {
        try {
            ...
            //验证账号是否登录次数过多被被锁定
            $lock_pwd = UserPwdManager::isUnlock($this->guard, $user->id);
            if($lock_pwd['state'] == 1){
                throw new LoginFailedException($lock_pwd['mess'], Code::LOGIN_FAILED_COUNT);
            }
            if (!password_verify($condition['password'], $user->password)) {
                //密码错误开关是否开启
                $lock_pwd =  UserPwdManager::UserPwd($this->guard, $user->id);
                if($lock_pwd['state'] == 1){
                  throw new LoginFailedException($lock_pwd['mess'], Code::LOGIN_FAILED_COUNT);
                }
                throw new LoginFailedException('登录失败|' . $user->username, Code::LOGIN_FAILED);
            }
            //登录成功进行密码次数解锁
            UserPwdManager::UnlockUserPwd($this->guard, $user->id);
            return $this->{$this->getDriver()}($user);

        } catch (\Exception $exception) {
            throw new LoginFailedException($exception->getMessage(), $exception->getCode());
            //
        }
    }
